...
<body>
    <div class="container">
        ...
        <div class="card">
            <h2>👀 或者直接搜索</h2>
            <input type="text" id="direct-search" class="search-input" placeholder="输入关键词直接搜索...">
            <button class="btn" style="margin-top: 12px; background: #667eea;" onclick="directSearch()">🔍 搜索</button>
    ...
    <script>
        const allQuestions = QUESTIONS_DATA_PLACEHOLDER;
        ...
        function directSearch() {
            ...
            results.slice(0, 20).forEach(q => {
                ...
                html += '<div style="background:#f8f9ff;padding:15px;border-radius:8px;margin-bottom:12px;border-left:4px solid #667eea;">';
                html += '<div style="margin-bottom:8px;">';
                html += '<span style="background:#667eea;color:white;padding:2px 8px;border-radius:4px;font-size:12px;margin-right:6px;">' + setName + '</span>';
                html += '<span style="background:#f5576c;color:white;padding:2px 8px;border-radius:4px;font-size:12px;">' + typeName + '</span>';
                html += '</div>';
                html += '<p style="color:#333;margin-bottom:10px;line-height:1.7;">' + q.question + '</p>';
                html += '<p style="color:#2e7d32;font-weight:bold;font-size:16px;">✅ 答案：' + answerDisplay + '</p>';
                html += '</div>';
            });
        ...
        function doSearch() {
            ...
            results.slice(0, 15).forEach(q => {
                ...
                html += '<div class="party-result-item">';
                html += '<div style="margin-bottom:8px;">';
                html += '<span class="party-badge party-badge-set">' + setName + '</span>';
                html += '<span class="party-badge party-badge-type">' + typeName + '</span>';
                html += '</div>';
                html += '<div class="party-result-q">' + q.question + '</div>';
                html += '<div class="party-result-a">✅ 答案：' + answerDisplay + '</div>';
                html += '</div>';
            });
            container.innerHTML = html;
        }
...
# 替换题目数据
final_html = html_template.replace('QUESTIONS_DATA_PLACEHOLDER', questions_json)
