...
for para in doc.paragraphs:
    ...
    # 检查是否是题目开始
    if text.startswith('1、') or text.startswith('2、') or text.startswith('3、') or text.startswith('4、') or text.startswith('5、') or text.startswith('6、') or text.startswith('7、') or text.startswith('8、') or text.startswith('9、') or text.startswith('10、'):
        # 保存上一个题目
        if current_question:
    ...
    # 检查是否是答案
    elif current_question and '正确答案：' in text:
        answer_part = text.split('正确答案：')[1].strip()
        # 处理判断题答案
