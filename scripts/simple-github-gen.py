...
# 步骤1：在 script 开头添加嵌入式数据
# 找到 <script> 标签后插入
script_tag = '<script>'
data_code = f'''
    // ========== 嵌入式题目数据（共 {len(questions)} 道）==========
    const allQuestionsData = {questions_js};
'''

# 替换 script 标签，在后面添加数据
html = html.replace(script_tag, script_tag + '\n' + data_code)

# 步骤2：删除或替换 API_BASE 定义
html = html.replace("const API_BASE = '/api';", '')

# 步骤3：替换所有 API 调用的函数
# 现在我们需要写完整的静态函数代码
# 让我们直接替换关键函数

# 首先，找到关键的函数开始位置，用我们的版本替换
...
# 现在我们需要找到原始代码中从变量定义开始到 script 结束的位置
# 最简单的办法：找到第一个 script 标签结束后是否有内容
# 或者直接在最后一个 </script> 前替换

# 找到原来的变量定义和函数，替换为我们的
# 先找到 window.onload 函数位置，修改它
...
new_onload = '''        window.onload = function() {
            ...
            console.log('✅ GitHub Pages 静态版已加载 ' + allQuestionsData.length + ' 道题目');
        };'''

html = html.replace(old_onload, new_onload)

# 现在，找到 script 结束标签，在前面插入我们的完整函数代码
# 但是要注意避免重复
# 让我们用一个标记方法：找到 </script>，把原来的内容替换为我们的函数 + </script>

# 先找到最后一个 </script> 位置
script_end = '</script>'

# 找到原来从变量定义开始到结束的内容，删除它们，替换为我们的完整代码
# 让我们简单一些：直接在 </script> 前插入我们的函数，同时确保原来的函数不会执行
# 更安全的做法：找到整个 <script> ... </script> 块，替换为新的

# 找到 script 开始和结束位置
# 这比较复杂，让我们用一个替换策略：
# 1. 我们已经在开头添加了数据
# 2. 现在我们需要用新的函数替换所有旧函数

# 为了简单起见，让我们把新函数直接添加在最后，并且通过函数名覆盖的方式来生效
# 但是我们需要避免重复的变量定义

# 简单但有效的办法：把 script 标签结束前的所有内容替换为我们的函数

# 让我们在 html 中找到 script 标签后到结束的部分，替换为我们的代码

# 为了简单，让我们直接输出最终的完整版本

# 让我们采用一个更直接的办法：在最后插入我们的函数，并且确保它们会覆盖旧函数
# 同时删除原来的一些重复定义

# 让我们简单一些：先找到 </script>，在那之前插入我们的完整函数代码
# 但要注意把旧的 API 函数都覆盖

# 让我们直接在 </script> 前添加我们的函数代码
# 同时，我们的函数名和旧函数名一样，这样会覆盖它们

html = html.replace(script_end, static_functions + '\n' + script_end)

# 现在保存为 index-github.html
with open('index-github.html', 'w', encoding='utf-8') as f:
    f.write(html)

print('完成！生成了 index-github.html，共', len(questions), '道题目')
